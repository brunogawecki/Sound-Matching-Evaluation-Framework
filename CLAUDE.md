...
*   `models/`: The Layer 3 model package. `base_model.py` defines the `BaseModel` ABC (`fit` / `predict` / `save` / `load`); `mean_parameter_baseline.py` (`MeanParameterBaseline`) is the naive train-set-mean floor (issue #7). No real model families implemented yet.
