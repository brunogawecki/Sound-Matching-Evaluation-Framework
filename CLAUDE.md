...
The framework is designed to programmatically generate audio datasets using headless VST synthesizers (currently supporting Dexed/DX7 and u-he Diva, with plans for Surge XT and TAL-NoiseMaker) and subsequently evaluate ML models against these rendered sounds.
...
*   `synth/`: Contains the synthesizer wrapper modules.
    ...
    *   `diva/synth.py`: `DivaWrapper`, the second synthesizer (u-he Diva, D-DIVA-START). Diva's plugin-reported names are not unique, so parameters are addressed by **module-qualified name** (`'VCF1.Model'`, `'LFO1.Rate'`) resolved through the static table in `diva/parameters.py` (VST3 never reports the owning module). DawDreamer-only (`renderers/pedalboard_renderer.py` shifts Diva's indices). Diva does not reproduce in-process at all — only fresh-process renders are valid (D-DIVA-RENDER) — and `render_audio` issues one discarded warm-up render first to absorb Diva's ~126 ms parameter-smoothing before the scored note starts.
    ...
    *   `diva/subset.py`: The Diva parameter subset (D-DIVA-SUBSET LOCKED): 237 of Diva's 281 real parameters (135 continuous + 102 one-hot = 1100 ML dimensions). Builds a `ParameterSpace` from a live `DivaWrapper` the same way `dexed/subset.py` does.
    *   `diva/h2p_param_map.py`: Committed table mapping each `.h2p` preset key (Diva's internal short code, scoped to a `#cm=<Module>` section — `'Atk'`, `'KeyFlw'`) onto the module-qualified parameter name plus how to decode its value (`linear` / `grid` / `label` / `index`). Derived by `scripts/derive_diva_h2p_map.py`, never recomputed at import time.
    *   `diva/patch.py`: `parse_h2p` (reads a `.h2p` file's `#cm=` sections, stopping at the binary tail) and `patch_parameters` (decodes through `h2p_param_map.py` into module-qualified names → normalized floats, the same shape every other Diva preset source produces).
    *   `plugin_output.py`: `suppressed_plugin_output` / `silence_plugin_output_from_now_on`, fd-level stdout+stderr suppression for plugins that log wider than Dexed's own `suppressed_stderr` (Diva writes to stdout as well as stderr, and logs from its destructor at process teardown).
...
*   `dataset/`: The Layer 2 data package. `builder.py` (`DatasetBuilder`) renders a `PresetSource` (`preset_sources.py`: synthetic / human / hybrid) into a self-describing corpus; `dexed_preset_loader.py` loads + dedups + splits DX7 cartridge voices; `diva_h2p_preset_loader.py` does the same for u-he's own installed `.h2p` preset library (factory + `THIRD PARTY`, 1432 presets, decoded via `synth/diva/patch.py`) — the default Diva human source (D-DIVA-START amendment), since it realizes the full 237-parameter subset where the older `diva_preset_loader.py` (the Flow Synthesizer `diva_raw.zip` collection, 11,218 `.npz` presets, name-matched onto `synth/diva/parameters.py`) realizes only 58; `preset_loader_common.py` holds the synth-agnostic half every loader shares (`LoadedPreset`, dedup, seeded train/test split, `restrict_to_realized` for the corpus-variance rule). `render_backends.py` holds `RenderSettings` + a registry-backed `make_wrapper(renderer, synth_name)` plus `FreshProcessRenderBackend` (fresh-process-per-render at pos 0 — D-REPRO) and parallel variants (`Parallel{Fresh,InProcess}RenderBackend`, opt-in via `--workers`, not validated against Diva). `torch_dataset.py` (`RenderedCorpusDataset`) consumes a corpus as `(audio, target)` pairs and rebuilds its `ParameterSpace` from `run_summary.json` with no live VST (D-SELFDESC). `scripts/build_dataset.py` takes `--synth {dexed,diva}` (default `dexed`) and, for Diva's `human`/`hybrid` subcommands, `--preset-format {h2p,npz}` (default `h2p`) on every subcommand; Diva always renders fresh-process (D-DIVA-RENDER) regardless of the `--fresh-process` flag. Generated corpora live under `dataset/<run_name>/` (gitignored).
*   `models/`: The Layer 3 model package. `base_model.py` defines the `BaseModel` ABC (`fit` / `predict` / `save` / `load`); `base_deep_model.py` (`BaseDeepModel`) adds the shared, Lightning-free `save`/`load`/`predict` for deep families; `mean_parameter_baseline.py` (`MeanParameterBaseline`) is the naive train-set-mean floor (issue #7); `sound2synth.py` (`Sound2SynthSpectrogramRegressor`, issue #19/#31) is the **first real deep family** — a VGG11-BN net over a log-power STFT emitting the ML-side vector through `ParameterSpace`. It is a deliberately *basic* first cut (single spectrogram branch + plain MLP head), **not** the paper's full multi-modal encoder / grouped-FC classifier, which remains future work. `registry.py` exposes `MODEL_REGISTRY` — name → (model class, default checkpoint filename) — the single source of truth `scripts/fit_model.py --model` and `scripts/evaluate.py --model` both read; a new family registers once and is trainable and evaluable everywhere. `models/presetgen_vae/` is the **generative family** (preset-gen-vae, Le Vaillant et al. DAFx 2021, issue #23): a subpackage with one file per role — `network.py` (mel-dB front-end + the paper's VAE with its latent RealNVP flow), `realnvp.py` (ported flow, used on the latent and as one family's regressor head), `families.py` (`PresetGenVAEMLPRegressor` / `PresetGenVAEFlowRegressor` — the paper's two reported models, differing only in the head), `lightning_module.py` (training-only loss recipe); see `docs/PRESETGEN_VAE_PORT.md`. `models/inversynth2/` is the **neural-proxy family** (InverSynth II, Barkan et al. ISMIR 2023): three staged models under the paper's own names — `IS` (encoder, parameters loss only), `IS2xITF` (adds a training-only differentiable synthesizer-proxy + audio loss; the "x" reads *excluding* ITF), and `IS2` (adds per-sample inference-time finetuning at predict time). `network.py` (encoder + training-only proxy), `families.py` (the three registered families), `lightning_module.py` (the Stage 2/3 combined-loss recipe); the proxy is training-only, so `predict` re-renders with the real Dexed (D-EVAL/D-REPRO); see `docs/INVERSYNTH2_PORT.md`. `models/flow_matching/` is the **conditional-generative flow-matching family** (Hayes, Saitis, Fazekas, ISMIR 2025, "Audio Synthesizer Inversion in Symmetric Parameter Spaces with Approximately Equivariant Flow Matching"): `flow_matching.py` (rectified-flow path, OT pairing, CFG, RK4 sampler), `encoder.py` (`AudioSpectrogramTransformer`), `vector_field.py` (the non-equivariant `ConditionalResidualMLPField` and the permutation-equivariant `EquivariantTransformerField`/Param2Tok), `network.py` (`FlowMatchingNetwork` composing mel front-end + encoder + field), `families.py` (`FlowMatchingMLP` / `FlowMatchingParam2Tok`), `lightning_module.py` (the flow-matching training recipe, ignores the injected `ParameterLoss`). `models/synthrl/` is the **reinforcement-learning family** (SynthRL, Shin & Lee IJCAI 2025): two of the paper's three staged models — `SynthRLp` (stage 1, parameter loss only) and `SynthRLi` (stage 2, in-domain RL, warm-started from a `SynthRLp` checkpoint via `--init-from`); stage 3 (`SynthRL-o`, out-of-domain) is deferred on D-FAMILIES. `representation.py` (the SynthRL-local per-parameter class-index view — every parameter becomes a classification head, numerical ones discretized onto 25 equally spaced classes — wrapping, never modifying, `ParameterSpace`), `network.py` (mel front-end → strided conv → transformer encoder → DETR decoder with one learnable query per parameter → per-parameter class heads), `families.py`, `lightning_module.py` (Gaussian-smoothed class cross-entropy; REINFORCE + reward-PER curriculum), `reward.py` / `reward_buffer.py`. The RL stage re-renders with the real Dexed **inside the training loop**, so a `SynthRLi` *training* run needs the plugin (training-only deviation from D-SELFDESC; `predict` and the eval path stay VST-free); see `docs/SYNTHRL_PORT.md`. Convention: a paper port that outgrows one file becomes such a subpackage. `models/training/` holds the shared PyTorch-Lightning training harness (issue #28, imported lazily so the eval path stays training-dependency-free — D-FRAMEWORK).
*   `evaluation/`: The Layer 4 evaluation package. `registry.py` exposes `METRIC_PANEL` — 13 per-sample `MetricSpecification`s across the parameter / magnitude / timbre / loudness / pitch axes (callables in `metrics/`; the embedding/perceptual axis is deferred — D-METRIC-PERCEPTUAL, issue #8). `evaluator.py` (`Evaluator`, issue #9) scores a fitted model on a corpus: per sample it predicts, re-renders the prediction fresh-process at pos 0, runs the panel, and writes `results/<corpus>/<model>/{per_sample.csv, eval_summary.json}` (render contract **and synth** read from the corpus, never `config.py` — D-EVAL; a corpus predating the second synth carries no `"synth"` key and defaults to Dexed). Outputs under `results/` and `checkpoints/` are gitignored.
*   `paper_repos/`: Contains legacy/reference code from related papers (e.g., `InverSynth2`, `preset-gen-vae`).
*   `dashboard/`: A private, localhost **Streamlit** control panel over the pipeline (issue #12). Four pages — Build dataset → Train on cluster → Evaluate → Results — that **subprocess** the `scripts/*.py` commands (build/evaluate) or shell out over SSH (training) and read their output files back for display; it never imports the pipeline library, so it cannot drift from the CLI. `script_specs.py` holds declarative `ArgSpec`/`ScriptSpec` tables mirroring each script's `argparse`; `forms.py` turns a spec into widgets + argv (`build_command` is the pure, tested core); `command_runner.py` streams stdout live (honoring `\r` for one-line `tqdm`); `cluster_runner.py` drives the PUT SLURM cluster (D-DASHBOARD-CLUSTER): pushes a corpus, `git pull`s the remote checkout, submits `cluster/train.sbatch`, tracks jobs in the local, gitignored `cluster/jobs.json` registry, and polls status/tails logs/cancels/pulls checkpoints for the Train on cluster page's live job view; `discovery.py` scans `config.py` paths for dropdown options, including which eval runs have saved prediction audio for the Results page's A/B Listen section. The D1 subset is shown read-only. Run with `streamlit run dashboard/app.py`. See `docs/ARCHITECTURE.md`.
