...
| `build_dataset.py` | CLI around `DatasetBuilder`: build a synthetic, human, or hybrid corpus. `--synth {dexed,diva}` (default `dexed`) picks the synthesizer; `--workers N` parallelizes fresh-process partitions across worker processes without changing the rendered audio. |
