...
| `presetgen_vae/` | The preset-gen-vae port (Le Vaillant et al. DAFx 2021, issues #23/#35/#36), a package with one file per role: `network.py` (mel-dB front-end + the paper's `speccnn8l1_bn` VAE), `realnvp.py` (plain-torch port of the paper's `CustomRealNVP` flow), `families.py` (the registered wrappers `PresetGenVAEMLPRegressor` / `PresetGenVAEFlowRegressor`), `lightning_module.py` (the training-only VAE loss recipe, lazily imported). Map: `docs/PRESETGEN_VAE_PORT.md`. Convention: a paper port that outgrows one file becomes such a subpackage. |
