...
class RenderBackend(abc.ABC):
    ...
    A RenderBackend hides one VST-hosting engine (DawDreamer, Pedalboard, ...) behind a
    ...
    wrapper above (e.g. DexedWrapper), so a wrapper works with any backend unchanged.

    The choice of backend is recorded per dataset/evaluation run: backends must never be mixed
    ...
    @abc.abstractmethod
    def name(self) -> str:
        """Short backend identifier (e.g. 'dawdreamer', 'pedalboard') for run metadata."""
        ...

    @property
    @abc.abstractmethod
    def sample_rate(self) -> int:
        """The sample rate at which this backend renders."""
        ...
