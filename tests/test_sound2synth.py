"""Tests for the spectrogram-CNN discriminative regressor (issue #19).

A forward-shape and rebuild-determinism check on the network, plus an end-to-end
fit -> save -> load -> predict smoke test on a tiny synthetic corpus (mirroring
``tests/test_training_smoke.py``). All variants use a shrunken conv schedule and a
small STFT so the VGG stack does not collapse the tiny spectrogram and the run stays
fast on CPU. Skips cleanly when ``torch``/``lightning`` are absent (training-only deps).
"""
import os
import sys
...
class FakeSynth:
    """A no-VST sine synth (mirrors tests/test_training_smoke.py)."""
...
def test_fit_export_load_predict_end_to_end(tmp_path):
    ...
    checkpoint_path = tmp_path / "spectrogram_cnn.pt"
...
def test_fit_defaults_wandb_run_name_to_model_and_corpus(tmp_path, monkeypatch):
    ...
    def spy_build_trainer(training_config, **kwargs):
        ...
        return real_build_trainer(training_config, **kwargs)
    ...
    assert captured["training_config"].logger.run_name == "Sound2SynthSpectrogramRegressor-train"
